 ...
 * <p>
 * Adapter for files extension list
 */

public class FileExtListAdapter extends RecyclerView.Adapter<FilesViewHolder> {

    private static final int MAX_ITEMS_TO_BE_DISPLAYED = 5;
    ArrayList<FileExt> fileSizeList = new ArrayList<>();

    public void swapAdapter(ArrayList<FileExt> files) {
        fileSizeList.clear();
        fileSizeList = files;
    ...
    public void onBindViewHolder(FilesViewHolder holder, int position) {
        FileExt file = fileSizeList.get(position);
        holder.bindExt(file.getFileName(), file.getFileSize());
    }
